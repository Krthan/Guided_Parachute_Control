...
final_BCs = [x_pos_final; y_pos_final; z_pos_final; u_final; v_final; w_final;...
    phi_final; theta_final; psi_final; p_final; q_final; r_final];
...
x0 = [x_pos_guess; y_pos_guess; z_pos_guess; u_guess; v_guess; w_guess; phi_guess;...
    theta_guess; psi_guess; p_guess; q_guess; r_guess; Cx_guess; Cy_guess; Cz_guess];
...
[x, fval] = fmincon(@(x) objective(x, n), x0, [], [], [], [], [], [], @(x) constraints(x, D, tf, n, BCs), options);
...
t1 = legslb(n);
tt1 = ((tf-t0).*tf + (tf+t0)./2);


[t, x_ode] = ode45(@(t,x) parachute_dynamics_w_inputs(t, x, tt1, Cx_result, Cy_result, Cz_result), tspan, initial_BCs);
